...
顯示模型、專案、context 用量、session 成本、**5 小時訂閱配額（含倒數）**，依方案顯示 7 天配額或月額度用量，再加 git / venv / token 統計。
...
- **Pro / Max 有 7 天限速** → `⏰ 5h 6% ⟳ 11m | 7d 2% ⟳ 6d16h`
- **Pro 有月額度（無 7 天限速）** → `⏰ 5h 10% ⟳ 3h40m | 💳 10% ($2.07/$20)`
- **兩者都沒有** → `⏰ 5h 6% ⟳ 11m`

數字直接從 Anthropic 的 OAuth `usage` endpoint 拿，跟 Claude Code TUI 裡 `/status` 的數字完全一致。
...
> 注意：`used_credits` 和 `monthly_limit` 單位是**分（cents）**（207 = $2.07，2000 = $20）。
...
| Reset 倒數符號 | `statusline.ps1` | `$h5Str` / `$quotaStr` 裡的 `⟳` 字元 |
...
- 5h 配額只對 Pro / Max 訂閱用戶有意義。純 API key 用戶會看到 `--`。
- `CLAUDE_CONFIG_DIR` 是 Claude Code 內部的 JavaScript 變數，不會 export 到子程序。腳本會自動 fallback 到 `$env:USERPROFILE\.claude`。
